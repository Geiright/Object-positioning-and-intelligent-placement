...
def auto_collection():
    ...
    camcoord1 = world2pixel_box(coord1)
    points.append(camcoord1)                            # coord1 三维坐标对应的像素坐标 camcoord1

    a = robot.new_movej_xyz_lr(coord1[0], coord1[1], coord1[2]+40, 0,120,0,1)      # 机械臂准备吸取  (盒子这边是1，物块那一边是-1)
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    time.sleep(0.5)  
                                   
    a = robot.new_movej_xyz_lr(coord1[0], coord1[1], coord1[2]-5, 0, 90, 0,1)         # 机械臂吸取
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a)) 
    time.sleep(0.5)  
    # coord1up = coordup(coord1)
    a = robot.new_movej_xyz_lr(coord1[0], coord1[1], coord1[2] + 40, 0, 110,0,1)       # 机械臂抬起
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    judgenum = judge()
...
def recollection():
    ...
    a = robot.new_movej_xyz_lr(coord1[0], coord1[1], coord1[2] + 50, rz1,140,0,-1)      # 机械臂准备吸取
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    time.sleep(0.5)  
                                   
    a = robot.new_movej_xyz_lr(coord1[0], coord1[1], coord1[2]-8, rz1,90, 0,-1)         # 机械臂吸取
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a)) 
    time.sleep(0.5)  

    a = robot.new_movej_xyz_lr(coord1[0], coord1[1], coord1[2] + 50, rz1,100,0,-1)       # 机械臂抬起
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    ...
    time.sleep(0.5) 
    print("ready to place: coord value {}, speed 70\n".format(coord2))
    a = robot.new_movej_xyz_lr(coord2[0], coord2[1], coord2[2] + 50, 0,120,0,1)    # 机械臂准备放置
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    time.sleep(0.5)
    a = robot.new_movej_xyz_lr(coord2[0], coord2[1], coord2[2]+4, 0,90,0,1)       # 机械臂放置
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    time.sleep(0.5)
    ...
    a = robot.new_movej_xyz_lr(coord2[0], coord2[1], coord2[2] + 50, 0,100,0,1)      # 机械臂抬起
    robot.wait_stop()
    if a == 1: print("moving") 
    else: print("error, code is {}".format(a))
    time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    for epoch in range(opt.epochs):
        ...
        a = robot.new_movej_xyz_lr(box_pos[0], box_pos[1], -34, 0, speed=130, roughly=0, lr=1)
        robot.wait_stop()
        print("robot statics is {}".format(a))
        time.sleep(0.5)
